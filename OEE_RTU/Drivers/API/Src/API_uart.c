...
HAL_UART_StateTypeDef uartSendString(uint8_t * pstring){
	if(pstring == NULL) return;
	size_t size = strlen((char*)pstring);
	return HAL_UART_Transmit_IT(&huart, (const uint8_t*)pstring , (uint16_t)size);
}

HAL_UART_StateTypeDef uartSendStringSize(uint8_t * pstring, uint16_t size){
	if(pstring == NULL) return;
	return HAL_UART_Transmit_IT(&huart, (const uint8_t*)pstring , size);
}

HAL_UART_StateTypeDef uartReceiveStringSize(uint8_t * pstring, uint16_t size){
	return HAL_UART_Receive_IT(&huart,pstring, size);
}
